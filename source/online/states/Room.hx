...
class Room extends MusicBeatState {
	...
	public function new() {
		...
		roomCodeTip = new FlxText(0, roomCode.y + roomCode.height + 5, FlxG.width, "(Press " #if android + "D" + #else + "S" + #end "to show your room code, CTRL+C to copy)");
		...
		chatBoxTip = new FlxText(0, 0, 0, "(Press " #if android + "C" + #else + "TAB" + #end "to show chat)");
    ...
    override function update(elapsed) {
		if (!chatBox.focused) {
			...
			if (FlxG.keys.justPressed.SPACE #if mobileC || virtualPad.buttonX.justPressed #end) {
			...
			if (FlxG.keys.justPressed.S #if mobileC || virtualPad.buttonZ.justPressed #end) {
			...
			if (FlxG.keys.justPressed.SHIFT #if mobileC || virtualPad.buttonY.justPressed #end) {
    ...
    function updateTexts() {
		...
		if (!GameClient.hasPerms()) {
			...
			#if mobileC
			roomVisibleTip.text = "(This can be changed in the settings (Y))";
			startGameTip.text = "(Press: A to Start Game; X to select song)";
			#else
