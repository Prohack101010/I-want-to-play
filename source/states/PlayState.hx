...
class PlayState extends MusicBeatState
{
	...
	override public function update(elapsed:Float)
	{
		...
		luaDebugGroup.forEachAlive(function(error:DebugLuaText){
			if(error.alpha == 0 || error.y >= FlxG.height) remove(error); error.destroy();
		});
		...
		if ((#if android FlxG.android.justReleased.BACK
			|| #end FlxG.keys.anyPressed(controls.keyboardBinds["pause"]))
			&& startedCountdown
			&& canPause)
		{
		...
		if (!GameClient.isConnected() && !ClientPrefs.data.noReset && controls.RESET && canReset && !inCutscene && startedCountdown && !endingSong)
		{
			health = 0;
			trace("RESET = True");
		}
	...
	public function updateScoreOpponent() {
		var op = (GameClient.isOwner ? GameClient.room.state.player2 : GameClient.room.state.player1);
